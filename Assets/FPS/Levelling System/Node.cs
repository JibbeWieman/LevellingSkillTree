...
public class Node : MonoBehaviour
{
    ...
    //private SkillUnlockedEvent SkillUnlocked = Events.SkillUnlockedEvent;
    ...
    private void Start()
    {
        ...
        //skillToolTip.tooltipTitle.text = skillName;
        //skillToolTip.tooltipDescription.text = skillDescription;
    }
    ...
    /*public void Unlock()
    {
        Debug.Log($"Trying to unlock. Current points: {SkillPointsChanged.SkillPoints}");
        if (!CanUnlock() || IsUnlocked)
        {
            return;
        }

        // Assume you have enough points to unlock
        IsUnlocked = true;
        SkillPointsChanged.SkillPoints -= cost;
        UpdateButtonColour();

        OnSkillUnlocked.skillType = skillType;
        EventManager.Broadcast(OnSkillUnlocked);
        //EventManager.Broadcast(SkillPointsChanged);
    }*/
    ...
    //    if (skillPoints.SkillPoints < cost) // Should I use an if statement or is this more optimal?
    //        return;

    //    Debug.Log($"Trying to unlock skill. Current points: {skillPoints.SkillPoints}");
    //    skillPoints.SkillPoints -= cost;
    //    Debug.Log($"Deducted {cost}. Current points amount: {skillPoints.SkillPoints}");

    //    IsUnlocked = true;

    //    #region CHANGE BUTTON COLOUR
    //    // Change used Node colour to yellow
    //    if (ColorUtility.TryParseHtmlString("#DBB448", out Color newColor))
    //    {
    //        ColorBlock colors = skillButton.colors;
    //        colors.normalColor = newColor; // Set the normalColor to the new custom color
    //        skillButton.colors = colors;

    //        // Force the button to update its UI immediately
    //        //skillButton.GetComponent<Graphic>().SetVerticesDirty();

    //        //Canvas.ForceUpdateCanvases();

    //        // Fix for immediately updating the colour
    //        skillButton.interactable = false;
